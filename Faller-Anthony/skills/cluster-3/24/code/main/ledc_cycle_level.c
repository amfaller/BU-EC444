...
*  Using PWM to Control Power to an LED - Auto Cycle
*  October 18, 2020
*  Author: Tony Faller */
...
void app_main(void)
{
    ...
    while (1) {
        ...
        switch(level){
            case 0:
                printf("Level 0\n");
                dutyEq = LEDC_TEST_DUTY_0;
                break;
            case 1:
                printf("Level 1\n");
                dutyEq = LEDC_TEST_DUTY_1;
                break;
            case 2:
                printf("Level 2\n");
                dutyEq = LEDC_TEST_DUTY_2;
                break;
            case 3:
                printf("Level 3\n");
                dutyEq = LEDC_TEST_DUTY_3;
                break;
            case 4:
                printf("Level 4\n");
                dutyEq = LEDC_TEST_DUTY_4;
                break;
            case 5:
                printf("Level 5\n");
                dutyEq = LEDC_TEST_DUTY_5;
                break;
            case 6:
                printf("Level 6\n");
                dutyEq = LEDC_TEST_DUTY_6;
                break;
            case 7:
                printf("Level 7\n");
                dutyEq = LEDC_TEST_DUTY_7;
                break;
            case 8:
                printf("Level 8\n");
                dutyEq = LEDC_TEST_DUTY_8;
                break;
            case 9:
                printf("Level 9\n");
                dutyEq = LEDC_TEST_DUTY_9;
                break;
            default:
        ...
        if(dutyEq > 4000) {dutyEq = 4000;}
        if(dutyEq < 0)    {dutyEq = 0;}

        /* Change LED Brightness with fade*/
        // printf("LEDC fade to duty = %d\n\n", dutyEq);
        // for (ch = 0; ch < LEDC_TEST_CH_NUM; ch++) {
        //     ledc_set_fade_with_time(ledc_channel[ch].speed_mode,
        //             ledc_channel[ch].channel, dutyEq, LEDC_TEST_FADE_TIME);
        //     ledc_fade_start(ledc_channel[ch].speed_mode,
        //             ledc_channel[ch].channel, LEDC_FADE_NO_WAIT);
        // }
        // vTaskDelay(LEDC_TEST_FADE_TIME / portTICK_PERIOD_MS);

        /* Change LED Brightness without fade */
        printf("LEDC set duty = %d without fade\n\n", dutyEq);
        for (ch = 0; ch < LEDC_TEST_CH_NUM; ch++) {
            ledc_set_duty(ledc_channel[ch].speed_mode, ledc_channel[ch].channel, dutyEq);
            ledc_update_duty(ledc_channel[ch].speed_mode, ledc_channel[ch].channel);
        }
        vTaskDelay(250 / portTICK_PERIOD_MS);
        

        /* Increment Level, wraparound */
        level++;
        if(level >= 10){
            level = 0;
        }
