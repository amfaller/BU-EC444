...
#include "./ADXL343.h"	// https://adafruit.github.io/Adafruit_ADXL343/html/_adafruit___a_d_x_l343_8h.html

/* Define marcors for i2c */
...
void calcRP(float x, float y, float z){
  ...
  negateX = -1 * xp;
  pitch = atan2(negateX,sqrt(yp*yp+zp*zp));
...
void app_main() {
  ...
  // Disable interrupts
  uint8_t val = 0;
  writeRegister(ADXL343_REG_INT_ENABLE, &val);
  ...
  // Enable measurements
  uint8_t measurements = 0x08;
  writeRegister(ADXL343_REG_POWER_CTL, &measurements);
